...
public class OrderDAO extends DBContext{
    public int insert(int memberID, String status, ArrayList<ItemOrder> itemsOrder){
        ...
        return executeUpdate("insert into OrderHistory (MemberID,Status,Time) values("+memberID+",N'"+status+"', CONVERT(datetime,'"+time+"'))")+new ItemOrderDAO().insert(getOrderBy(memberID, status, date).getOrderID(), itemsOrder);
    }
    
    public int update(int orderID, int memberID, String status){
        return executeUpdate("update OrderHistory set Status=N'"+status+"', Time=CONVERT(datetime,'"+LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"))+"') where OrderID="+orderID+" and MemberID="+memberID);
    }
    ...
    public ArrayList<Order> getListOrderBy(int memberID,String order){
        return getListOrder("select * from OrderHistory where MemberID="+memberID+" order by "+order);
    }
