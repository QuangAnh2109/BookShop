...
public class BookDAO extends DBContext{
    public int insert(Book book){
        ...
        return executeUpdate("insert into Book (Name,Quantity,Price,Discount,Introduce,NumberPages,Weight,Size,PublishingYear,AuthorName,CompanyID,CoverTypeID,CategoryID) values('"+book.getName()+"',"+book.getQuantity()+","+book.getPrice()+","+book.getDiscount()+",'"+book.getIntroduce()+"',"+book.getNumberPages()+","+book.getWeight()+",'"+book.getSize()+"',"+book.getPublishingYear()+",'"+book.getAuthorName()+"',"+book.getCompany().getId()+","+book.getType().getId()+","+book.getCategory().getId()+")");
    }
    
    public boolean delete(Book book){
        String sql = "delete from Book where BookID="+book.getId();
        try {
            connection.prepareStatement(sql).executeUpdate();
            new CompanyDAO().delete(book.getCompany().getId());
            new CoverTypeDAO().delete(book.getType().getId());
            new CategoryDAO().delete(book.getCategory().getId());
            return true;
    ...
    public int update(Book book){
        ...
        book.setCategory(categoryDAO.getCategoryBy(book.getCategory()));
        book.setType(coverTypeDAO.getCoverTypeBy(book.getType()));
        String sql = "update Book set Name = '"+book.getName()+"' , Quantity = "+book.getQuantity()+", Price = "+book.getPrice()+", Discount = "+book.getDiscount()+", Introduce = '"+book.getIntroduce()+"' , NumberPages = "+book.getNumberPages()+", Weight = "+book.getWeight()+", Size = '"+book.getSize()+"' , PublishingYear = "+book.getPublishingYear()+", AuthorName = '"+book.getAuthorName()+"' , CompanyID = "+book.getCompany().getId()+", CoverTypeID = "+book.getType().getId()+", CategoryID = "+book.getCategory().getId()+" where BookID = "+book.getId();
        try {
    ...
    public ArrayList<Book> getListBookBy(String searchName, ArrayList<String> companys, ArrayList<String> categorys, ArrayList<String> covers){
